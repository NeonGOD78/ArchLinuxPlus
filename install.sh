...
# ==================== Setup UKI dracut timer ====================

setup_uki_dracut_timer() {
  section_header "Installing UKI Auto-Rebuild Timer with Dracut"

  local timer_dir="/mnt/etc/systemd/system"
  local script_path="/mnt/usr/local/bin/rebuild-uki"
  local output_path="/efi/EFI/Linux/arch.efi"
...
EOF
  ...
  cat <<EOF > "$timer_dir/uki-rebuild.service"
[Unit]
...
EOF
  ...
  cat <<EOF > "$timer_dir/uki-rebuild.timer"
[Unit]
...
EOF
  ...
  startup_ok "UKI dracut-based rebuild timer installed and enabled."
}
